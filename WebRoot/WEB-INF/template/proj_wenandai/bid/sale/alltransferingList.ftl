...
<!--start: Container -->
    <div class="per-box my-per-box">
      <div class="per">
    	...
    	<#--<span class="setp">|</span>-->
	   <#-- <a href="${base}/creditFlow/financingAgency/alltransferinglistVipPlBidSale.do" class="<#if planType=="ortransfer">bg2<#else>bg1</#if>">VIP债权交易</a>-->
   	  </div>
				...
				<input type="hidden" id="pageSize" name="pager.pageSize" value="${pager.pageSize}"/>
				    <div class="filter-condition">
					      ...
					      <ul class="filter-content"  id="proKeepType"> 
						      ...
						      <li><a href="javascript:void(0);" class="attribute" lang="0-3">3个月以内</a><li>
  ...
  <#if planType=="ortransfer">
        	...
        	<div class="investment-list">
        		<div class="fl w1015">
	        		<div class="investment-dl" >       			
	        			...
	        			<div class="f16">出让本金：<#if list.saleMoney==0>0.00<#else><#if list.saleMoney lt 1000>${list.saleMoney}<#else>${list.saleMoney?string(',###.00')}</#if></#if>元</div>
	        		</div>
	        		<div class="investment-dl">
	        			<div class="f16">折让金：<#if list.changeMoney=0> ${list.changeMoney}<#else> <#if list.changeMoneyType=0> +<#else>-</#if><#if list.changeMoney==0>0.00<#else><#if list.changeMoney lt 1000>${list.changeMoney}<#else>${list.changeMoney?string(',###.00')}</#if></#if></#if>元</div>
	        			<div class="f16">折让率：<#if list.changeMoneyRate=0> ${list.changeMoneyRate} <#else> <#if list.changeMoneyType=0> +<#else>-</#if>${list.changeMoneyRate}</#if>‰</div>
	        		</div>
	        		<div class="investment-dl">
	        			<div class="f16">挂牌时间：${list.saleStartTime?string("yyyy-MM-dd HH:mm:ss")}</div><!---->
        			</div>
	        	</div>
	        	<div class="bid_list_btn fr">
	        		 <#if list.saleStatus==3>
	                          <span  class="gray-btn buttongray">正在交易</span>
	                        <#else>
	                          <a href="${base}/creditFlow/financingAgency/saleInfoPlBidPlan.do?salseId=${list.id}" class="buttonorange orange-btn">购买中</a> 
	                 </#if>  
        		</div>
   ...
   <#--<div class="bidplan1">
                <table class="tab_css_0">
                    ...
                    <tr>
                        <td align="left">
	                       <em><img style="padding-left:42px" src="${base}/theme/${systemConfig.theme}/images/<#if (list.proKeepType)??><#if list.proKeepType=="信用审核标">xin.png<#elseif list.proKeepType=="实地核查标">shi.png<#elseif  list.proKeepType="机构担保标">bao.png<#elseif list.proKeepType="福利标">fu.png<#else>xin.png</#if><#else>xin.png</#if>" width="19"/></em>
	                        <a href="${base}/creditFlow/financingAgency/saleInfoPlBidPlan.do?salseId=${list.id}" title="${list.bidProName}(到期日:${list.intentDate};年化利率:${list.yearAccrualRate}%)">
	                       <#if  list.bidProName?length gt 10> ${list.bidProName?substring(0,10)}<#else> ${list.bidProName}</#if>  </a>
                        </td>
        ...
        <div class="investment">
        	...
        	<div class="investment-list">
        		<div class="fl w1015">
	        		...
	        		<div class="investment-dl">
	        			<div class="f16">计划金额${list.progress}：${list.sumMoney?string(',##0')}元</div>
	        			<div class="f16">
	        				<span>投资进度：</span>
		        			<span class="progress project-progress w160" style="margin-top:9px;">
							    <span class="progress-bar progress-bar-info" role="progressbar" aria-valuenow="11.00" aria-valuemin="0" aria-valuemax="10" style="width: ${list.progress}%"></span>
							</span>
							<span class="progress-num" style="color:#262626;">${list.progress}%</span>
	        			</div>
        			</div>
	        	</div>
	        	<div class="bid_list_btn fr">
