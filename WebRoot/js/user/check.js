...
　　(7)12位移动电话号码的第二位和第三位为"13"
 ...
 function isAmountErr(str) {
	 ...
	 //afterM 标的剩余金额
	 //currM 我的账户余额
	 function chkMoney(idval,afterM,currM) {
	 	var object=$("#"+idval);
	 	var currM=$("#"+currM).val();
		...
		var setMoney = $("#setMoney").text();
		if(mySignMoney!=""){
			if(Number(mySignMoney)<Number(setMoney)){
				$("#ratioMoney").text(Number(mySignMoney)*returnRatio/100);
			}else{
				$("#ratioMoney").text(Number(setMoney)*returnRatio/100);
			}
		}else{
			$("#ratioMoney").text(Number(setMoney)*returnRatio/100);
		}
		...
		if(object.val() == ""){
			var money_span= $("#money_span").text();
			if(money_span==null||money_span==""){
			 	$("#money_span").html("<span style='font-size:12px;'>金额不能为空。</span>");
			 	$("#moneyspan").html("<span style='font-size:12px;'>金额不能为空。</span>");
			 	return false;
			}
		} else if (!flag) {
	        $("#money_span").html("<span style='font-size:12px;'>金额必须大于0且为整数或小数。</span>");
	        $("#moneyspan").html("<span style='font-size:12px;'>金额必须大于0且为整数或小数。</span>");
	        object.focus();
         	return false;
        }else if(parseInt(object.val())> parseInt($("#upperMoney").val())){
     		$("#money_span").html("<span style='font-size:12px;'>您输入的金额大于了当前标的投资上限。</span>");
     		$("#moneyspan").html("<span style='font-size:12px;'>您输入的金额大于了当前标的投资上限。</span>");
         	object.focus();
        	return false;
    	} else {
	     	if(parseInt(object.val())>parseInt(afterM)){
	     		$("#money_span").html("<span style='font-size:12px;'>您输入的投资金额大于标的剩余金额。</span>");
	     		$("#moneyspan").html("<span style='font-size:12px;'>您输入的投资金额大于标的剩余金额。</span>");
	     		return false;
	     	}else if (parseInt(object.val())>parseInt(currM)){
	     		$("#money_span").html("<span style='font-size:12px;'>您输入的投资金额大于您的账户余额。</span>");
	     		$("#moneyspan").html("<span style='font-size:12px;'>您输入的投资金额大于您的账户余额。</span>");
	     		return false;
	     	}else{
	     		$("#money_span").html("");
	     	 	$("#moneyspan").html("");
...
/*statrM：起投金额，riseM：递增整数倍,yhsyl:年化收益率，day:项目期限*/
 function chkMoney1(idval,afterM,currM,statrM,yhsyl,day) {
     var object=$("#"+idval);
     /*var currM=$("#"+currM).val();*/
     var currM=$("#"+currM).text();
     if(currM !==""){
         currM = currM.replace(',','')
     }
     ...
     if(object.val() == ""){
         ...
         object.focus();
         return false;
     }else if(parseInt(object.val())<statrM){
         $("#money_span").html("<span style='font-size:12px;'>您输入的金额小于了起投金额。</span>");
         $("#moneyspan").html("<span style='font-size:12px;'>您输入的金额小于了起投金额。</span>");
         object.focus();
         return false;
     }else if(parseInt(object.val())> parseInt($("#upperMoney").val())){
         $("#money_span").html("<span style='font-size:12px;'>您输入的金额大于了当前标的投资上限。</span>");
         ...
         if(parseInt(object.val())>parseInt(afterM)){
            ...
            /* if(parseInt(object.val())%riseM){
                 var val=parseInt(parseInt(object.val())/riseM)*riseM;
                 object.val(val);
             }*/
             var day1;
             if(day.indexOf('年')>-1){
                 day1=parseInt(day)*365;
             }else if(day.indexOf('月')>-1){
                 day1=parseInt(day)*30;
             }else{
                 day1=parseInt(day);
             }
             var yhsyl1=yhsyl/100;
             var yqsy=(yhsyl1/365*object.val()*day1).toFixed(2);
             $("#moneyspan").html("<span style='font-size:12px;'>预期收益：<span style='color:red;'>"+yqsy+"</span>元（不含募集期利息，收益以实际为准）</span>");
             //$("#money_span").html("");
             //$("#moneyspan").html("");
             return true;
         }
