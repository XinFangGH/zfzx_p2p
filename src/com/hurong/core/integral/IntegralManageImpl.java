	...
	//	Dictionary dictionary = dictionaryService.get(bpActivityManage.getSendType());
		...
		try {
			synchronized (obj) {
				...
				if(bpCustMember.getScore()==null||bpCustMember.getScore().compareTo(new Long(0))==0){//没有积分时
					bpCustMember.setScore(score);
...
//				}
	...
	public void integralEngine(String userAction, String userActionKey) {
		...
		try {
			...
			Object object = null;
			if(session!=null){
				object = session.getAttribute(MyUserSession.MEMBEER_SESSION);
				if(object==null){
					return ;
				}
			}else{
			...
			if(list!=null&&list.size()>0){
				...
				for(int i = 0 ; i<list.size() ; i++){
					WebBonusSetting webBonusSetting = list.get(i);
					webBonusSettingDao.evict(webBonusSetting);
					//如果会员要求不是无则进行查找会员要求所对应的分数
					if(webBonusSetting.getMemberLevel()!=null&&!"0".equals(webBonusSetting.getMemberLevel())){
			...
			for(int i=0 ; i < arrList.size()-1 ; i++){
				for(int j=i+1 ; j<arrList.size(); j++ ){
					WebBonusSetting temp = list.get(i);
			...
			WebBonusSetting webBonusSetting = null;
			BpCustMemberService bpCustMemberService =(BpCustMemberService) AppUtil.getBean("bpCustMemberService");
			BpCustMember _bpCustMember = bpCustMemberService.get(bpCustMember.getId());
			bpCustMemberService.evict(_bpCustMember);
			if(bpCustMember.getScore()==null){
				_bpCustMember.setScore(Long.valueOf(0));
			}
			for(WebBonusSetting setting : arrList){
				//如果用户积分大于等于  并且  积分规则已开启则 返回这个规则
				if((_bpCustMember.getScore().compareTo(Long.valueOf(setting.getMemberLevelValue()))>=0)
			...
			if(webBonusSetting!=null){
				//查出积分规则后进行积分规则计算加分
				 addScordBySetting(_bpCustMember.getId(), webBonusSetting);
			}

		} catch (NullPointerException e) {
			System.out.println("没获得session。。");
		}
	}
