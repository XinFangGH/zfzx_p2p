...
import java.util.*;
     ...
     *
     */
    public class WebBidInfoAction extends BaseAction {
        @Resource
        ...
        public String biding() {
            ...
            String bidId = this.getRequest().getParameter("bidId");
            PlBidInfo pl = new PlBidInfo();
            if(bidId != null & !"".equals(bidId)) {
                List<PlBidPlan> list = plBidPlanService.getPlanDetail(Long.parseLong(bidId));
                if(list.size()>0){
                    pl.setPlBidPlan(list.get(0));
                    pl.setBidId(list.get(0).getBidId());
                    pl.setBidName(list.get(0).getBidProName());
                }
            }
            if(bpCustMember != null){
                pl.setUserId(bpCustMember.getId());
                pl.setUserName(bpCustMember.getLoginname());

            }
            String money = this.getRequest().getParameter("userMoney");
            if(money != null & !"".equals(money)){
                BigDecimal userMoney = new BigDecimal(money);
                pl.setUserMoney(userMoney);
            }
...
//                    boolean chkIsRepeat=chkRepeat();
                    ...
                    if(chkIsRepeat){
                        ...
                        if (pl != null) {
                            QueueManger.QueueOffer(pl);
                        }
                        pl = (PlBidInfo) QueueManger.QueuePeek();
                        //判断优惠券是否过期
                        if(couponId!=null&&!couponId.equals("")&&!couponId.equals("null")){
                            ...
                            if(coupon!=null){
                                ...
                                if(coupon.getCouponEndDate().getTime()<d.getTime()){
                                    ...
                                    this.getRequest().setAttribute("code", Constants.CODE_FAILED);
                                    this.getRequest().setAttribute("mes","所选择的优惠券已经过期,不能使用该优惠券！");
                                    if (isApp) {
                                        return "error_mes";
                                    }
                        ...
                        //判断是否符合投标递增情况
                        String[] chkBidArr=check(pl.getBidId(), pl);
                        PlBidPlan bidplan = plBidPlanService.get(pl.getBidId());
...
//                            webMsgInstance("0", Constants.CODE_FAILED, chkBidArr[2],"", "", "", "", "");
                            this.getRequest().setAttribute("code",Constants.CODE_FAILED);
                            this.getRequest().setAttribute("mes",chkBidArr[2]);
                            if (isApp) {
                                return "error_mes";
                            }
                            return "error_message";
                            ...
                            BpCustMember LoanMember=plBidInfoService.getLoanMember(pl);
                            String loanMoney=plBidInfoService.getLoanMoney(pl);
...
//                                webMsgInstance("0", Constants.CODE_FAILED, "投资人和借款人不能是同一人","", "", "", "", "");
                                ...
                                mobile=this.getRequest().getParameter("isApp");
                                ...
                                cq.setBidId(pl.getPlBidPlan().getBidId().toString());//标的号
                                cq.setFreezeOrdId(freezeOrdId);//冻结流水号
                                cq.setAmount(pl.getUserMoney().setScale(2));//投标金额
                                ...
                                //保存投标的信息
                                pl.setUserId(cpCut.getId());
                                pl.setUserName(cpCut.getLoginname());
                                pl.setBidName(bidplan.getBidProName());
                                if(couponId!=null&&!couponId.equals("")&&!couponId.equals("null")){
                                    pl.setCouponId(Long.valueOf(couponId));
                                }
                                if(bidplan.getBidRemark()!=null){
                                    pl.setPreAuthorizationNum(bidplan.getBidRemark());
                                }
                                pl.setState(PlBidInfo.TYPE0);
                                pl.setOrderNo(orderNum);
                                pl.setBidtime(new Date());
                                plBidInfoService.save(pl);
...
//                                webMsgInstance("0", Constants.CODE_FAILED, "投资人未开通第三方在支付","", "", "", "", "");
                                this.getRequest().setAttribute("code", Constants.CODE_FAILED);
                                this.getRequest().setAttribute("mes","投资人未开通第三方支付");
                                if (isApp) {
                                    return "error_mes";
                                }
                                return "error_message";
...
//                        webMsgInstance("0", Constants.CODE_FAILED, "不能重复提交","", "", "", "", "");
                        this.getRequest().setAttribute("code",Constants.CODE_FAILED);
                        this.getRequest().setAttribute("mes","不能重复提交");
                        if (isApp) {
                            return "error_mes";
                        }
                        return "error_message";
...
//                    webMsgInstance("0", Constants.CODE_FAILED, usercondition[1].toString(),"", "", "", "", "");
                    this.getRequest().setAttribute("code",Constants.CODE_FAILED);
                    this.getRequest().setAttribute("mes",usercondition[1].toString());
                    if (isApp) {
                        return "error_mes";
                    }
                    return "error_message";
...
//                webMsgInstance("0", Constants.CODE_FAILED, "系统错误，请联系管理员","", "", "", "", "");
                this.getRequest().setAttribute("code",Constants.CODE_FAILED);
                this.getRequest().setAttribute("mes","系统错误，请联系管理员");
                if (isApp) {
                    return "error_mes";
                }
                return "error_message";
