...
public class WebPhoneEmailAction extends BaseAction{
    @Resource
    ...
    public String appEmailBind() {
        ...
        if (mem != null) {
            ...
            if (bpemail == null) {
                ...
                String returnpath = getBasePath() + "webPhone/appUpdateEmailTypeWebPhoneEmailAction.do?emailcode=" + emailcode;
                ...
                try {
                    if (isSuccess) {
                        ...
                        if("1".equals(isApp)){
                            model.setMsg("邮件发送成功，请及时登录邮箱完成验证！");
                            setJsonString(model.toJSON());
                            return SUCCESS;
                        }
                        //webMsgInstance("0", Constants.CODE_SUCCESS, "邮件发送成功，请及时登录邮箱完成验证！", "", "", "", "", "");
                        model.addDataContent("status",1);
                        model.addDataContent("msg","邮件发送成功，请及时登录邮箱完成验证！");
                        setJsonString(model.toJSON());
                        return SUCCESS;
                    } else {
                        //设置 返回提示消息
                        ...
                        model.addDataContent("status",2);
                        model.setCode(MobileErrorCode.SERVICE_ERROR);
                        model.addDataContent("msg","邮件发送失败！");
                        setJsonString(model.toJSON());
                        return SUCCESS;
                ...
                if("1".equals(isApp)){
                    model.setCode(MobileErrorCode.SERVICE_ERROR);
                    model.setMsg("该邮箱已经被注册！");
                    setJsonString(model.toJSON());
                    return SUCCESS;
                }
                //设置 返回提示消息
                model.setCode(MobileErrorCode.SERVICE_ERROR);
                model.addDataContent("status",4);
                model.addDataContent("msg","该邮箱已经被注册！");
                setJsonString(model.toJSON());
                return SUCCESS;
            ...
            model.setCode(MobileErrorCode.SERVICE_ERROR);
            model.addDataContent("status",5);
            model.addDataContent("msg","请先登录！");
            setJsonString(model.toJSON());
            return SUCCESS;
        }
    }
    ...
    public boolean sendMail(String msgURL, String code) {
        ...
        data.setCode(code);
        data.setSite(msgURL);
        System.out.println(data.getCode()+"     code");
        System.out.println(data.getSite()+"     code");
     ...
     * 会员邮箱验证
     */
    public String appUpdateEmailType() {
        MobileDataResultModel model = new MobileDataResultModel();
        String isApp = this.getRequest().getParameter("isApp");
        try {
                    ...
                    if (ret.length > 1) {
                        ...
                        if (funntime <= 30) {
                            try {
                                ...
                                if("1".equals(isApp)){
                                    model.addDataContent("msg","恭喜您邮箱验证成功！");
                                    setJsonString(model.toJSON());
                                    return SUCCESS;
                                }
                                //设置 返回提示消息
    ...
    public String appEmailUpdate(){
        return "bindEmail1";
    }

    public String appEmailLogin(){
        return "login";
    }
