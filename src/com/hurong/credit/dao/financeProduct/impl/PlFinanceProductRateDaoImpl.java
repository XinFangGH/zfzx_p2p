...
public class PlFinanceProductRateDaoImpl extends BaseDaoImpl<PlFinanceProductRate> implements PlFinanceProductRateDao{
	...
	public List<PlFinanceProductRate> getAllRateAndOrder(HttpServletRequest request,PagingBean pb) {
		...
		String sql="SELECT "+
						...
						"(p.yearRate*100)/365 as thousandsIncome  "+
					"FROM "+
		...
		if(intentDateStart!=null&&!"".equals(intentDateStart)){
			...
			Object intentDateStarts=request.getAttribute("intentDateStart");
			if(intentDateStarts!=null&&!"".equals(intentDateStarts)){
				sql=sql+" and p.intentDate>= '"+intentDateStarts+" 00:00:00'";
			}
		}
		...
		if(intentDateEnd!=null&&!"".equals(intentDateEnd)){
			...
			Object intentDateEnds=request.getAttribute("intentDateEnd");
			if(intentDateEnds!=null&&!"".equals(intentDateEnds)){
				sql=sql+" and p.intentDate<= '"+intentDateEnds+" 59:59:59'";
			}
		}
		...
		List<PlFinanceProductRate> list=this.getSession().createSQLQuery(sql).
										...
										addScalar("thousandsIncome", Hibernate.BIG_DECIMAL).
