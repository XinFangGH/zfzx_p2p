...
import com.hurong.credit.util.FuiouUtil.SecurityUtils;
...
public class FuiouServiceImpl implements FuiouService {
	 ...
	 */
	@Override
	public Fuiou webRegister(BpCustMember mem, String basePath, HttpServletRequest request,WebBankcard webBankcard) {
		// TODO Auto-generated method stub
		String[] ret=new String[2];
		Fuiou fuiou=new Fuiou();
		try{
			fuiou=generatePublicData(fuiou);
			fuiou=regWebDate(fuiou,mem,request,webBankcard,basePath);
			Map<String, String> params =regWebParmasMap(fuiou);
			String sign=autoCreatSignnature(params,Fuiou.CHARSETUTF8);
			params.put("signature", sign);
			fuiou.setSignature(sign);
			System.out.println("url==="+configMap.get("thirdPay_fuiou_URL").toString()+Fuiou.FUIOUWEBREGEDIT);
			fuiou.setUrl(configMap.get("thirdPay_fuiou_URL").toString()+Fuiou.FUIOUWEBREGEDIT);
		}catch(Exception e){
			e.printStackTrace();
		}
		return fuiou;	
	 ...
	 * @return
	 */
	private Map<String, String> regWebParmasMap(Fuiou fuiou) {
		Map<String, String> params = new HashMap<String, String>();
			...
			params.put("user_id_from",fuiou.getUser_id_from());
			params.put("bank_nm",fuiou.getBank_nm());
			params.put("page_notify_url",fuiou.getPage_notify_url());
			params.put("back_notify_url",fuiou.getBack_notify_url());
		return params;
	}
	/**
	 * 网页个人主动注册数据储备
	 ...
	 * @return
	 */
	private Fuiou regWebDate(Fuiou fuiou, BpCustMember mem,
			HttpServletRequest request, WebBankcard webBankcard,String basePath) {
		// TODO Auto-generated method stub
		String	orderNum=Common.getRandomNum(3)+ DateUtil.dateToStr(new Date(), "yyyyMMddHHmmss");
		fuiou.setMchnt_txn_ssn(orderNum);//线上投资客户主键Id
		fuiou.setUser_id_from(mem.getId().toString());
		fuiou.setCust_nm(mem.getTruename());
		fuiou.setCertif_id(mem.getCardcode());
		fuiou.setMobile_no(mem.getTelphone());
		if(webBankcard!=null){
			fuiou.setCity_id(webBankcard.getCityId().toString());//北京市
			fuiou.setParent_bank_id(webBankcard.getBankId());//工商银行
			fuiou.setBank_nm(webBankcard.getSubbranchbank().trim());
			fuiou.setCapAcntNo(StringUtil.stringURLEncoderByUTF8(webBankcard.getCardNum()));
		}else{
			fuiou.setCity_id("");//北京市
			fuiou.setParent_bank_id("");//工商银行
			fuiou.setBank_nm("");
			fuiou.setCapAcntNo("");
		}
		fuiou.setEmail(mem.getEmail());
		fuiou.setPage_notify_url(basePath+"pay/getFuioureturnDatePay.do");
		fuiou.setBack_notify_url("");
		return fuiou;
	}
	...
	public String[] register(HttpServletResponse respose, BpCustMember mem,String regType, String basePath,HttpServletRequest request,WebBankcard webBankcard) {
		...
		Fuiou fuiou=new Fuiou();
		fuiou=generatePublicData(fuiou);
		fuiou=regDate(fuiou,mem,request,webBankcard);
	 ...
	 * @param webBankcard 
	 */
	public Fuiou regDate(Fuiou fuiou,BpCustMember bp,HttpServletRequest request, WebBankcard webBankcard){
		String	orderNum=Common.getRandomNum(3)+ DateUtil.dateToStr(new Date(), "yyyyMMddHHmmss");
		fuiou.setMchnt_txn_ssn(orderNum+bp.getId().toString());//线上投资客户主键Id
	...
	private Fuiou currentMemberMessage(Fuiou fuiou, BpCustMember bpCustMember) {
		...
		String	orderNum=Common.getRandomNum(3)+ DateUtil.dateToStr(new Date(), "yyyyMMddHHmmss");
		fuiou.setMchnt_txn_ssn(orderNum+"-"+bpCustMember.getId().toString());
		fuiou.setMchnt_txn_dt(sfDate.format(new Date()));
		fuiou.setUser_ids(bpCustMember.getThirdPayFlag0());//登录账号
		return fuiou;
	}
...
//			fuiou.setCapAcntNo(capAcntNo);//账号
			    ...
			    if(resp_code!=null&&resp_code.equals("0000")){
					...
					String  signNature=doc.selectSingleNode("/ap/signature").getText();
				    	...
				    	if(cu_balance!=null&&!"".equals(cu_balance)){//未转结余额
				    		bpCustMember.setUnChargeMoney((new BigDecimal(cu_balance)).divide(new BigDecimal(100)));
				    	}else{
    ...
    */
	...
	private Fuiou dealInfoQueryDate(Fuiou fuiou, String userIds, Date startDay,
		...
		String orderNum = Common.getRandomNum(3)
		+ DateUtil.dateToStr(new Date(), "yyyyMMddHHmmss");
		fuiou.setMchnt_txn_ssn(orderNum+userIds);
		...
		try{
			 ...
			 //得到signNature签名数据
			  ...
			  if(resp_code.equals("0000")){
				  String contract_no=doc.selectSingleNode("/ap/plain/contract_no").getText();
				  ret[0]=Constants.CODE_SUCCESS;
				  ret[1]=resp_code;
	...
	public Fuiou publicPreCancData(Fuiou fuiou,String mchnt_txn_ssn,String out_cust_no,String in_cust_no,String contract_no,String rem){
		 ...
		 * rem：备注
		 */
		public String[] transferPersonToPerson(BpCustMember bpCustMember,String outCustNo,String inCustNo,String  amt,String contractNo,String rem){
			String[] ret=new String[2];
			Fuiou fuiou=new Fuiou();
			fuiou=generatePublicData(fuiou);//传入商户代码字段
			fuiou=transferPersonToPersonDate(fuiou,bpCustMember, outCustNo, inCustNo,  amt, contractNo, rem);//把需要的参数值复制给fuion
		 ...
		 * @return
		 */
		public  Fuiou  transferPersonToPersonDate(Fuiou fuiou,BpCustMember bp,String outCustNo,String inCustNo,String  amt,String contractNo,String rem){
			
			String orderNum = Common.getRandomNum(3)
			+ DateUtil.dateToStr(new Date(), "yyyyMMddHHmmss");
	    	System.out.println("orderNum=="+orderNum);
			fuiou.setMchnt_txn_ssn(orderNum);//流水号
			fuiou.setOut_cust_no("13623478908");//收款
			fuiou.setIn_cust_no("18612610238");//付款
			fuiou.setAmt("10000");//转帐金额
			fuiou.setContract_no("000001387976");//授权合同号
			fuiou.setRem("");//备注
			return fuiou;
		}
		 ...
		 */
		@Override
		public String[] traAcc(){
			String[] ret=new String[2];
			Fuiou fuiou=new Fuiou();
			//得到公共的商务代码
			fuiou=generatePublicData(fuiou);
			//得到相应的属性
			fuiou=publicTraAccData(fuiou);
			//属性值存储在map<key,value>
		 ...
		 * @return
		 */
		public Fuiou publicTraAccData(Fuiou fuiou){
		return fuiou;
		}
