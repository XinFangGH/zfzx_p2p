...
public class SXTMessageStrategyImpl implements MessageStrategy {

	private static Log logger = LogFactory.getLog(SXTMessageStrategyImpl.class);
	...
	public String sendMsg(String phone,String code,String content) {
		...
		if(null!=projType&&!"".equals(projType)){
			///hurong/systemConfig/thirdPayConfig
			/*url = AppUtil.getThridMessageConfig("/interface/message/"+projType+"/url");
			userID = AppUtil.getThridMessageConfig("/interface/message/"+projType+"/userID");
			accountID = AppUtil.getThridMessageConfig("/interface/message/"+projType+"/accountID");
			password = AppUtil.getThridMessageConfig("/interface/message/"+projType+"/password");*/
		...
		//return directSend("http://www.mxtong.net.cn/GateWay/Services.asmx/DirectSend?",parseDTO(phone,code,content));
	}
	//获取参数
	...
	public String directSend(String url, DirectSendDTO directSendDTO) {
		...
		try {
			...
			logger.info("发送时间："+sdf.format(new Date())+" ,手机号："+phone+" ,验证码："+code+" ,短信发送.....!");
			int statusCode = httpClient.executeMethod(getMethod);
			if (statusCode != HttpStatus.SC_OK) {
				System.err.println("Method failed: "
						+ getMethod.getStatusLine());
			...
			logger.info("返回时间："+sdf.format(new Date())+" ,手机号："+phone+" ,验证码："+code+" ,短信发送成功!"+retCodeValue);
