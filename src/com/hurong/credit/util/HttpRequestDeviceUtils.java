...
public class HttpRequestDeviceUtils {
    ...
    public static String isMobileDevice1(HttpServletRequest request){
        ...
        String userAgent = request.getHeader("user-agent");
//        String isApp = request.getHeader("isApp");
         boolean isApp=  isAppBrower(request);
        for (int i = 0; via!=null && !via.trim().equals("") && i < mobileGateWayHeaders.length; i++) {
            if(via.contains(mobileGateWayHeaders[i])){
                mobileFlag = true;
                break;
        ...
        if(mobileFlag==true && pcFlag==false && !isApp){
            b="1";   //手机h5
        }
        if(mobileFlag==true && pcFlag==false && isApp){
            b="2"; //app
        }
        return b;//0 pc  1 h5   2app
     ...
     * @return
     */
     public static boolean isAppBrower(HttpServletRequest request){
         String isApp = request.getHeader("isApp");
         String userAgent = request.getHeader("user-agent");
         return "1".equals(isApp) ||userAgent.contains("zxzbAndroid") ||userAgent.contains("zxzbiOS");
     }
}
