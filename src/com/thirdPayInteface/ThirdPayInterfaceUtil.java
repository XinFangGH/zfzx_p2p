...
public class ThirdPayInterfaceUtil {
	 ...
	 * @return
	 */
	public static Object[] bidLoan(String thirdPayFlagId,String orderNum,String amount,String bidId){
		Date date = new Date();
		CommonRequst common = new CommonRequst();
		common.setRequsetNo(orderNum);
		common.setTransactionTime(date);
	...
	public static Object[] queryCustmerInfo(CommonRequst commonRequst) {
		commonRequst.setBussinessType(ThirdPayConstants.BUSSINESSTYPE_QUERY);
		return ThirdPayMethodCall(commonRequst);
	}
		...
		if(thirdPayType.equals(ThirdPayConstants.THIRDPAYTYPE0)){//资金托管模式
			if(thirdPayConfig.equals(ThirdPayConstants.SINAPAY)){//新浪支付
				if(commonRequst.getBussinessType().equals(ThirdPayConstants.BUSSINESSTYPE_REGISTER)){//注册
					Object[] sinaPay=SinaPayIntefaceUtil.register(commonRequst);
					ret[0]=sinaPay[0];
					ret[1]=sinaPay[1];
					ret[2]=sinaPay[2];
				}else {
				...
				if(commonRequst.getBussinessType().equals(ThirdPayConstants.BUSSINESSTYPE_REGISTER)){//注册
					...
					ret[1]=umpay[1];
					ret[2]=umpay[2];
				}else if(commonRequst.getBussinessType().equals(ThirdPayConstants.BUSSINESSTYPE_QUERY)){//查询
					Object[] umpay = UMPayInterfaceUtil.queryCustmerInfo(commonRequst);
					ret[0]=umpay[0];
					ret[1]=umpay[1];
					ret[2]=umpay[2];
				}
