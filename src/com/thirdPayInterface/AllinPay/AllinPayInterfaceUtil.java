...
import com.thirdPayInterface.AllinPay.UnionPay.DemoBase;
import com.thirdPayInterface.AllinPay.UnionPay.UnionPay;
import com.thirdPayInterface.AllinPay.UnionPay.UnionPayUtil;
...
public class AllinPayInterfaceUtil {
	 ...
	 * @return
	 */
	public static CommonResponse rechargeUnionPay(CommonRequst commonRequst){
		...
		//基础参数
		unionpay.setVersion(thirdPayConfig.get("unionpay_version").toString());
		unionpay.setEncoding("UTF-8");
		unionpay.setSignMethod("01");
		unionpay.setTxnType("01");
		unionpay.setTxnSubType("01");
		unionpay.setCurrencyCode("156");
		unionpay.setReqReserved("unionpay");
		unionpay.setMerId(thirdPayConfig.get("unionpay_merId").toString());
		//业务类型
		unionpay.setBizType(thirdPayConfig.get("unionpay_bizType").toString());
		//渠道类型 07-PC，08-手机
		unionpay.setChannelType(thirdPayConfig.get("unionpay_channelType").toString());
		//通知地址
		unionpay.setFrontUrl(BasePath+thirdPayConfig.get("thirdPay_pageCallUrl").toString()+"?isMobile=1");
		unionpay.setBackUrl(BasePath+thirdPayConfig.get("thirdPay_notifyUrl").toString()+"?isMobile=1");
		//接入类型，商户接入填0 0- 商户 ， 1： 收单， 2：平台商户
		unionpay.setAccessType(thirdPayConfig.get("unionpay_accessType").toString());
		//订单号
		unionpay.setOrderId(commonRequst.getRequsetNo());
		//订单发送时间
		unionpay.setTxnTime(new SimpleDateFormat("yyyyMMddHHmmss").format(new Date()));
		//交易金额
		unionpay.setTxnAmt(commonRequst.getAmount().multiply(new BigDecimal("100")).setScale(0).toString());
//		unionpay.setTxnAmt("1");//测试使用1分钱
		// 生成注册map
		Map<String, String> params = ObjectFormatUtil.createMapNoCass(UnionPay.class,unionpay);
		//数据签名
		UnionPayUtil.sign(params,AppUtil.getAppAbsolutePath()+thirdPayConfig.get("unionpay_signCertPath").toString(),thirdPayConfig.get("unionpay_signCertPwd").toString(),thirdPayConfig.get("unionpay_signCertType").toString(), "UTF-8");
		String [] rett=ThirdPayWebClient.operateParameter(thirdPayConfig.get("unionpay_frontTransUrl").toString(), params,"UTF-8");
	 ...
	 * @return
	 */
	public static CommonResponse queryRechargeUnionPay(CommonRequst commonRequst){
		...
		try {
			...
			unionpay.setEncoding("UTF-8");
			unionpay.setSignMethod("01");
			unionpay.setTxnType("00");
			unionpay.setTxnSubType("00");
			unionpay.setMerId(thirdPayConfig.get("unionpay_merId").toString());
			unionpay.setBizType("000000");
			unionpay.setAccessType("0");
			//订单号
			unionpay.setOrderId(commonRequst.getQueryRequsetNo());
			//订单发送时间
			unionpay.setTxnTime(new SimpleDateFormat("yyyyMMddHHmmss").format(new Date()));
			Map<String, String> params = ObjectFormatUtil.createMapNoCass(UnionPay.class,unionpay);
			UnionPayUtil.sign(params,AppUtil.getAppAbsolutePath()+thirdPayConfig.get("unionpay_signCertPath").toString(),thirdPayConfig.get("unionpay_signCertPwd").toString(),thirdPayConfig.get("unionpay_signCertType").toString(), "UTF-8");
			Map<String, String> resmap = DemoBase.submitUrl(params, thirdPayConfig.get("unionpay_queryTransUrl").toString());
			System.out.println("银联交易状态查询返回的参数="+resmap);
			if(resmap!=null){
				if(resmap.get("respCode").equals("00")){
					if(resmap.containsKey("origRespCode")){
						if(resmap.get("origRespCode").equals("00")){
							response.setResponsecode(CommonResponse.RESPONSECODE_SUCCESS);
							List<CommonRecord> re = new ArrayList<CommonRecord>();
							CommonRecord record = new CommonRecord();
							record.setStatus(CommonResponse.RESPONSECODE_SUCCESS);
							re.add(record);
							response.setRecordList(re);
							response.setResponseMsg("查询交易成功");
						}else{
							response.setResponsecode(CommonResponse.RESPONSECODE_APPLAY);
							response.setResponseMsg(resmap.get("origRespMsg"));
						}
					}else{
						response.setResponsecode(CommonResponse.RESPONSECODE_APPLAY);
					...
					response.setResponseMsg(resmap.get("respMsg"));
				}
			}else{
				response.setResponsecode(CommonResponse.RESPONSECODE_FAILD);
				response.setResponseMsg("未查询到任何信息");
			}
		} catch (Exception e) {
			response.setResponsecode(CommonResponse.RESPONSECODE_FAILD);
			response.setResponseMsg("银联交易查询对接失败！");
			e.printStackTrace();
		}
		return response;
	}
	...
	public static CommonResponse withdraws(CommonRequst commonRequst) {
		...
		//交易批次号
		String reqSn="hry-"+DF_merchantId+"-"+commonRequst.getRequsetNo();
...
//		XmlTools.initProvider(provider);
		...
		if(result[0].equals("0000")){
			...
			requst.setQueryRequsetNo(commonRequst.getRequsetNo());
			response = queryResult(requst);
		}else{
			response.setResponsecode(CommonResponse.RESPONSECODE_FAILD);
			response.setResponseMsg("提现申请失败:"+result[1]);
		}
		response.setDealState("7");
		return response;
	}
	/**
	 * 批量代付 交易结果查询
	...
	public static CommonResponse queryResult(CommonRequst commonRequst){
		...
		info.setVERSION("04");
		info.setDATA_TYPE("2");
		info.setREQ_SN("hry-"+DF_merchantId+"-"+commonRequst.getQueryRequsetNo());
		...
		ret_detail.setQUERY_SN("hry-"+DF_merchantId+"-"+commonRequst.getQueryRequsetNo());
	...
	public void batchDaiShou(String url,boolean isTLTFront) throws Exception {
		...
		String reqSn="hry-"+DF_merchantId+"-"+String.valueOf(System.currentTimeMillis());
		InfoReq info=makeReq(trxCode,reqSn,thirdPayConfig);
		aipg.setINFO(info);
		Body body = new Body() ;
		Trans_Sum trans_sum = new Trans_Sum() ;
...
//		trans_detail3.setSUMMARY("分组清算");
	 ...
	 * @return
	 */
	public static boolean unionpayVerify(Map<String, String> respParam,HttpServletRequest req){
		//得到证书路径
		Map thirdPayConfig=allinPayProperty();
		String certDir=thirdPayConfig.get("unionpay_verifySignAcpCer").toString();
		certDir=AppUtil.getAppAbsolutePath()+certDir;
		...
		try {
			...
			if (null != reqParam && !reqParam.isEmpty()) {
				...
				while (it.hasNext()) {
					...
					if(!key.equals("isMobile")){
						valideData.put(key, value);
					}
				}
			}
			if (!UnionPayUtil.validate(valideData, encoding)) {
