...
public class YeePayInterfaceUtil {
	 ...
	 * @return
	 */
	public static Map yeePayProperty(){
	...
	public static YeePay generatePublicData(boolean pageCallBack, boolean notifyCallBack,boolean isMobile) {
		...
		if(thirdPayConfig!=null){
			...
			if(pageCallBack){
				if(isMobile){
					yeepay.setCallbackUrl(BasePath+thirdPayConfig.get("thirdPay_yeepay_callbackUrl").toString().trim()+"?isMobile=1");//页面回跳 URL
				}else{
...
//==================================接口开始=========================================
	...
	public static CommonResponse register(CommonRequst commonRequst){
		...
		try{
			//公共商户号
			YeePay yeepay=generatePublicData(true,true,false);
			if(commonRequst.getIsMobile()!=null&&!"".equals(commonRequst.getIsMobile())&&"1".equals(commonRequst.getIsMobile())){
				 yeepay=generatePublicData(true,true,true);
			}else{
				 yeepay=generatePublicData(true,true,false);
			}
			if(yeepay!=null){
				...
				String[] rett=null;
				if(commonRequst.getIsMobile()!=null&&!"".equals(commonRequst.getIsMobile())&&("1".equals(commonRequst.getIsMobile()))){
					rett = ThirdPayWebClient.operateParameter(yeepayurl+YeePay.YEEPAYTOREG_MOBILE, params,YeePay.CHARSETUTF8);
				}else{
					rett = ThirdPayWebClient.operateParameter(yeepayurl+YeePay.YEEPAYTOREG, params,YeePay.CHARSETUTF8);
				}
		...
		try{
			...
			if(yeepay!=null){
				...
				if(commonRequst.getThreeCardCode()!=null&&!commonRequst.getThreeCardCode().equals("")){
					yeepay.setUnifiedCode(commonRequst.getThreeCardCode());//三证合一
				}else{
				...
				yeepay.setMemberClassType(commonRequst.getMemberClassType());
				...
				String[] rett = null;
				if(commonRequst.getIsMobile()!=null&&!"".equals(commonRequst.getIsMobile())&&("1".equals(commonRequst.getIsMobile()))){
					rett=ThirdPayWebClient.operateParameter(yeepayurl+YeePay.ENTERPRISEREGISTER_MOBILE, params,YeePay.CHARSETUTF8);
				}else{
					rett=ThirdPayWebClient.operateParameter(yeepayurl+YeePay.ENTERPRISEREGISTER, params,YeePay.CHARSETUTF8);
				}
				logger.info("用户开户注册调用易宝参数："+rett[1]);
				if(rett!=null&&ThirdPayConstants.RECOD_SUCCESS.equals(rett[0])){//
					commonResponse.setResponsecode(CommonResponse.RESPONSECODE_APPLAY);
		...
		try{
			...
			if(yeepay!=null){
				...
				if(commonRequst.getIsMobile()!=null&&("1".equals(commonRequst.getIsMobile()))){
					rett=ThirdPayWebClient.operateParameter(yeepayurl+YeePay.YEEPAYTORECH_MOBILE, params,YeePay.CHARSETUTF8);
				}else{
					rett=ThirdPayWebClient.operateParameter(yeepayurl+YeePay.YEEPAYTORECH, params,YeePay.CHARSETUTF8);
				}
				if(rett!=null&&ThirdPayConstants.RECOD_SUCCESS.equals(rett[0])){//
					commonResponse.setResponsecode(CommonResponse.RESPONSECODE_APPLAY);
...
//				}
				...
				String [] rett=null;
				if(commonRequst.getIsMobile()!=null&&!"".equals(commonRequst.getIsMobile())&&("1".equals(commonRequst.getIsMobile()))){
					 rett=ThirdPayWebClient.operateParameter(yeepayurl+YeePay.TOWITHDRAW_MOBILE, params,YeePay.CHARSETUTF8);//手机端取现
				}else{
					 rett=ThirdPayWebClient.operateParameter(yeepayurl+YeePay.TOWITHDRAW, params,YeePay.CHARSETUTF8);
				}
				if(rett!=null&&ThirdPayConstants.RECOD_SUCCESS.equals(rett[0])){//
					commonResponse.setResponsecode(CommonResponse.RESPONSECODE_APPLAY);
	...
	public  static CommonResponse toBindBankCard(CommonRequst commonRequst) {
		...
		try{
			...
			if(yeepay!=null){
				...
				String [] rett = null;
				if(commonRequst.getIsMobile()!=null&&!"".equals(commonRequst.getIsMobile())&&("1".equals(commonRequst.getIsMobile()))){
					 rett=ThirdPayWebClient.operateParameter(yeepayurl+YeePay.TOBINDBANKCARD_MOBILE, params,YeePay.CHARSETUTF8);
				}else{
					 rett=ThirdPayWebClient.operateParameter(yeepayurl+YeePay.TOBINDBANKCARD, params,YeePay.CHARSETUTF8);
				}
				if(rett!=null&&ThirdPayConstants.RECOD_SUCCESS.equals(rett[0])){//
					commonResponse.setResponsecode(CommonResponse.RESPONSECODE_APPLAY);
	...
	public  static CommonResponse cancelBindBankCard(CommonRequst commonRequst) {
		...
		try{
			...
			if(yeepay!=null){
				yeepay.setPlatformUserNo(commonRequst.getThirdPayConfigId());//商户平台会员标识
				yeepay.setRequestNo(commonRequst.getRequsetNo());//绑定银行卡流水号
				Map<String, String> params=getReqSign(yeepay,"UNBIND_CARD");
				System.out.println(params);
				//易宝支付调用地址
				String yeepayurl=yeePayUrl();
				//取消绑卡直连接口更改
				...
				if(response.getCode().equals("1")){
					...
					commonResponse.setResponsecode(CommonResponse.RESPONSECODE_FAILD);
					commonResponse.setResponseMsg(response.getDescription());
					commonResponse.setCommonRequst(commonRequst);
				}
				logger.info("用户取消绑定银行卡调用易宝参数：");
			}else{
				commonResponse.setResponsecode(CommonResponse.RESPONSECODE_FAILD);
				commonResponse.setResponseMsg("基本参数获取失败");
				commonResponse.setCommonRequst(commonRequst);
	...
	public static CommonResponse autoTransferAuthorization(CommonRequst commonRequst) {
		...
		try{
			...
			if(null != yeepay){
				...
				String[] rett=null;
				if(commonRequst.getIsMobile()!=null&&!"".equals(commonRequst.getIsMobile())&&("1".equals(commonRequst.getIsMobile()))){
					 rett=ThirdPayWebClient.operateParameter(yeepayurl+YeePay.YEEPAYTOAUTHORIZEAUTOTRANSFER_MOBILE, params,YeePay.CHARSETUTF8);
				}else{
					 rett=ThirdPayWebClient.operateParameter(yeepayurl+YeePay.YEEPAYTOAUTHORIZEAUTOTRANSFER, params,YeePay.CHARSETUTF8);
				}
	...
	public static CommonResponse fiancialTransfer(CommonRequst commonRequst) {
		...
		try{
			...
			if(yeepay!=null){
				...
				yeepay.setRemark("用户投标");
				String acmount = "";
				if(commonRequst.getBussinessType().equals(ThirdPayConstants.BT_MMUSER)){//理财计划
					//判断是否有加入费用
					if(commonRequst.getFee().compareTo(new BigDecimal("0"))>0){
						acmount = commonRequst.getAmount().subtract(commonRequst.getFee()).toString();
						Detail detailFee = new Detail();
						detailFee.setAmount(commonRequst.getFee().toString());//交易金额
						...
						acmount = commonRequst.getAmount().toString();
					}
				}else{//散标
					acmount = commonRequst.getAmount().toString();
				}
				Detail detail = new Detail();
				detail.setAmount(acmount);//交易金额
				...
				p.setProperty("tenderDescription", "投标");//项目描述
				...
				Map<String, String> params=getReqSign(yeepay,null);
				//String paramstr = params.toString().replace("</request>", str+"</request>");
				//易宝支付调用地址
				String yeepayurl=yeePayUrl();
				String [] rett=null;
				if(commonRequst.getIsMobile()!=null&&!"".equals(commonRequst.getIsMobile())&&("1".equals(commonRequst.getIsMobile()))){
					 rett=ThirdPayWebClient.operateParameter(yeepayurl+YeePay.YEEPTOTRANSFER_MOBILE, params,YeePay.CHARSETUTF8);
				}else{
					rett=ThirdPayWebClient.operateParameter(yeepayurl+YeePay.YEEPAYCOMMONTTRANSFER, params,YeePay.CHARSETUTF8);
				}
				cr.setRequestInfo(rett[1]);
				cr.setReturnType(CommonResponse.RETURNTYPE_HTML);
				if(rett!=null&&ThirdPayConstants.RECOD_SUCCESS.equals(rett[0])){
	 ...
	 * @return
	 */
	public static CommonResponse toRepaymentByLoaner2(CommonRequst commonRequst){
	...
	public static CommonResponse autoRepaymentAuthorization(CommonRequst commonRequst){
		...
		try{
			...
			if(null != yeepay){
				...
				String[] rett=null;
				if(commonRequst.getIsMobile()!=null&&!"".equals(commonRequst.getIsMobile())&&("1".equals(commonRequst.getIsMobile()))){
					 rett=ThirdPayWebClient.operateParameter(yeepayurl+YeePay.YEEPAYTOAUTHORIZEAUTOREPAYMENT_MOBILE, params,YeePay.CHARSETUTF8);
				}else{
					 rett=ThirdPayWebClient.operateParameter(yeepayurl+YeePay.YEEPAYTOAUTHORIZEAUTOREPAYMENT, params,YeePay.CHARSETUTF8);
				}
				//cr.setRequestInfo(rett[1]);
				cr.setReturnType(CommonResponse.RETURNTYPE_HTML);
	...
	public static CommonResponse obligationDeal(CommonRequst commonRequst) {
		...
		try{
			...
			if(yeepay!=null){
				...
				yeepay.setPaymentRequestNo(commonRequst.getQueryRequsetNo());
				Map<String, String> params=getReqSign(yeepay,null);
				System.out.println(params);
				//易宝支付调用地址
				String yeepayurl=yeePayUrl();
	 ...
	 * @return
	 */
	public static CommonResponse obligationDeal2(CommonRequst commonRequst) {
		logger.info("调用易宝支付网关用户债权交易接口开始时间："+new Date());
		CommonResponse cr=new CommonResponse();
		try{
			//公共商户号
			...
			if(yeepay!=null){	
				...
				yeepay.setRequestNo(commonRequst.getRequsetNo());//新的投标流水号
				yeepay.setUserType("MEMBER");
				yeepay.setBizType("CREDIT_ASSIGNMENT");
				//封装资金明细记录
				List<Detail> details = new ArrayList<Detail>();
				Detail detail = new Detail();
				detail.setAmount(commonRequst.getAmount().toString());//交易金额
				detail.setTargetUserType("MEMBER");//用户类型,默认
				detail.setTargetPlatformUserNo(commonRequst.getLoaner_thirdPayflagId());//债权出让人
				detail.setBizType("CREDIT_ASSIGNMENT");
				details.add(detail);
				yeepay.setDetails(details);
				yeepay.setRemark("用户债权转让");
				//项目信息
				Properties p = new Properties();
				p.setProperty("tenderOrderNo",commonRequst.getBidType()+"-"+commonRequst.getBidId());//项目编号，根据标的类型和标的id拼接生成
				p.setProperty("creditorPlatformUserNo", commonRequst.getLoaner_thirdPayflagId());//债权出让人
				p.setProperty("originalRequestNo", commonRequst.getQueryRequsetNo());//需要转让的投资记录流水号
				String str = strProperty(p);
				yeepay.setExtend(str);
				...
				if(commonRequst.getIsMobile()!=null&&!"".equals(commonRequst.getIsMobile())&&("1".equals(commonRequst.getIsMobile()))){
					rett=ThirdPayWebClient.operateParameter(yeepayurl+YeePay.YEEPAYCOMMONTTRANSFER, params,YeePay.CHARSETUTF8);
					 ...
					 rett=ThirdPayWebClient.operateParameter(yeepayurl+YeePay.YEEPAYCOMMONTTRANSFER_MOBILE, params,YeePay.CHARSETUTF8);
				}
				cr.setReturnType(CommonResponse.RETURNTYPE_HTML);
				if(rett!=null&&ThirdPayConstants.RECOD_SUCCESS.equals(rett[0])){
					cr.setResponsecode(CommonResponse.RESPONSECODE_APPLAY);
	...
	/**
	 * 网关接口12--用户通用转账授权接口（给平台转账方法）1.0接口
	 * @param map
	 * @return
	 */
	...
	/**
	 * 用户通用转账授权接口（用户给平台转账方法）2.0接口
	 * @param map
	 * @return
	 */
	public static CommonResponse  ommontransferIntrface2(CommonRequst commonRequst){
		logger.info("调用易宝支付网关用户通用转账授权接口（给平台转账方法）开始时间："+new Date());
		CommonResponse cr=new CommonResponse();
		try{
			//公共商户号
			...
			if(yeepay!=null){
				...
				if(commonRequst.getDetailList()!=null){//是否存放了转账列表
					...
					for(CommonDetail temp:commonRequst.getDetailList()){
						Detail detailFee = new Detail();
						detailFee.setAmount(temp.getAmount().toString());//交易金额
						detailFee.setTargetUserType("MERCHANT");//用户类型,默认
						detailFee.setTargetPlatformUserNo(yeepay.getPlatformNo());//收款用户编号
						detailFee.setBizType("TRANSFER");
						details.add(detailFee);
					}
					yeepay.setRemark("债权挂牌交易手续费");
					yeepay.setDetails(details);
					Map<String, String> params=getReqSign(yeepay,null);
					System.out.println(params);
					//易宝支付调用地址
		...
		try{
			...
			if(yeepay!=null){
				...
				if(commonRequst.getRepaymemntList()!=null){//判断代偿还款list是否为空
					...
					for(CommonRequestInvestRecord temp:commonRequst.getRepaymemntList()){
						Transfer rep=new Transfer();
						rep.setTargetPlatformUserNo(temp.getInvest_thirdPayConfigId());
						rep.setTargetUserType("MEMBER");
						if(null!=commonRequst && "1".equals(commonRequst.getGuaranteeWay()))//本金保障，只偿还本金
						{
						  rep.setAmount(temp.getPrincipal().add(temp.getPrincipalAccMoney()).toString());
						}else if(null!=commonRequst && "2".equals(commonRequst.getGuaranteeWay()))//本息偿还，本金+利息+罚息
						{
							rep.setAmount(temp.getPrincipal().add(temp.getInterest()).add(temp.getInterestAccMoney()).toString());
						}else if(null!=commonRequst && "3".equals(commonRequst.getGuaranteeWay()))//全部保障，本金+利息+罚息+两项服务费
						{
							rep.setAmount(temp.getPrincipal().add(temp.getInterest()).add(temp.getAccrual()).add(temp.getFee()).toString());
							rep.setFee(temp.getFee().toString());
						}
					    transfers.add(rep);
					}
					yeepay.setTransfers(transfers);
					Map<String, String> params=getReqSign(yeepay,null);
					System.out.println(params);
					//易宝支付调用地址
					String yeepayurl=yeePayUrl();
	 ...
	 * @return
	 */
	public static CommonResponse guanteeRepayMent2(CommonRequst commonRequst) {
		logger.info("调用易宝支付网关用户还款接口开始时间："+new Date());
		CommonResponse cr=new CommonResponse();
		try{
			//公共商户号
			...
			if(yeepay!=null){
				...
				yeepay.setRequestNo(commonRequst.getRequsetNo());//投标流水号
				yeepay.setUserType("MEMBER");
				yeepay.setBizType("COMPENSATORY");//业务类型
				//封装资金明细记录
				if(commonRequst.getRepaymemntList()!=null){//判断还款list是否为空
					List<Detail> details = new ArrayList<Detail>();
					for(CommonRequestInvestRecord temp:commonRequst.getRepaymemntList()){
						...
						if(null!=commonRequst && "1".equals(commonRequst.getGuaranteeWay()))//本金保障，只偿还本金
						{
							amount = temp.getPrincipal().add(temp.getPrincipalAccMoney()).toString();
						}else if(null!=commonRequst && "2".equals(commonRequst.getGuaranteeWay()))//本息偿还，本金+利息+罚息
						{
							amount = temp.getPrincipal().add(temp.getInterest()).add(temp.getInterestAccMoney()).toString();
						}else if(null!=commonRequst && "3".equals(commonRequst.getGuaranteeWay()))//全部保障，本金+利息+罚息+两项服务费
						{
							amount = temp.getPrincipal().add(temp.getInterest()).add(temp.getAccrual()).toString();
							//服务费用
							if(temp.getFee().compareTo(new BigDecimal("0"))>0){
								Detail detailFee = new Detail();
								detailFee.setAmount(temp.getFee().toString());//交易金额
						...
						detail.setTargetUserType("MEMBER");//用户类型,默认
						detail.setTargetPlatformUserNo(temp.getInvest_thirdPayConfigId());//收款用户编号
						detail.setBizType("COMPENSATORY");
						details.add(detail);
					}
					yeepay.setRemark("担保公司代偿");
					yeepay.setDetails(details);
				}
				//项目信息
				Properties p = new Properties();
		...
		try{
			...
			if(yeepay!=null){
				...
				if(rett!=null&&ThirdPayConstants.RECOD_SUCCESS.equals(rett[0])){
					...
					cr.setResponsecode(CommonResponse.RESPONSECODE_FAILD);
					cr.setResponseMsg("用户重置交易密码申请失败");
				}
	...
	public static CommonResponse toResetMobile(CommonRequst commonRequst){
		...
		try{
			...
			if(yeepay!=null){
				...
				String [] rett=null;
				if(commonRequst.getIsMobile()!=null&&!"".equals(commonRequst.getIsMobile())&&("1".equals(commonRequst.getIsMobile()))){
					 rett=ThirdPayWebClient.operateParameter(yeepayurl+YeePay.YEEPAYTORESETMOBILE_MOBILE, params,YeePay.CHARSETUTF8);
				}else{
					 rett=ThirdPayWebClient.operateParameter(yeepayurl+YeePay.YEEPAYTORESETMOBILE, params,YeePay.CHARSETUTF8);
				}
	...
	public static CommonResponse registerQuery(CommonRequst commonRequst) {
		...
		try{
			//公共商户号
			YeePay yeepay=generatePublicData(false,true,false);
			if(yeepay!=null){
				yeepay.setPlatformUserNo(commonRequst.getThirdPayConfigId());//商户平台会员标识
				...
				if(yeepayurl!=null){
					...
					logger.info("注册用户查询结果："+outStr);
					YeePayReponse response =JAXBunmarshal(outStr,YeePayReponse.class);
					System.out.println("返回的银行信息"+response.getBank());
					...
					if(response.getCode().equals("1")){
						 if(response.getBank()!=null){
							...
							commonResponse.setPaySwift(response.getPaySwift());
						 }
						 ...
						 String custmemberStatus=response.getActiveStatus().equals("ACTIVATED")?"已激活":"未激活状态";
	...
	public static CommonResponse singleQuery(CommonRequst commonRequst) {
		...
		try{
			...
			if(yeepay!=null){
				...
				String mode="";
				if(commonRequst.getQueryType().equals("REPAYMENT_RECORD")){
					mode="CP_TRANSACTION";
				}else{
					mode = commonRequst.getQueryType();
				}
				yeepay.setMode(mode);//查询类型
	...
	public static CommonResponse cancelTransferAuthorization(CommonRequst commonRequst) {
		...
		try{
			//公共商户号
			YeePay yeepay=generatePublicData(false,false,false);
			cr.setCommonRequst(commonRequst);
			if(null != yeepay){
				yeepay.setPlatformUserNo(commonRequst.getThirdPayConfigId());//商户平台会员标识
				yeepay.setRequestNo(commonRequst.getRequsetNo());//请求流水号
     ...
     * 直连接口08--通用转账授权审核接口 
     * @param map
     * @return  false yeepay.setMode=CANCEL
     */
		...
		try{
			...
			if(yeepay!=null){
				...
				if(yeepayurl!=null){	
					...
					YeePayReponse response =JAXBunmarshal(outStr,YeePayReponse.class);
					if(response.getCode().equals("1")){
						commonResponse.setResponsecode(ThirdPayConstants.RECOD_SUCCESS);
						commonResponse.setResponseMsg("业务对账成功");
						commonResponse.setYeepayreponse(response);
					}else{
						commonResponse.setResponsecode(ThirdPayConstants.RECOD_FAILD);
						commonResponse.setResponseMsg("业务对账失败:"+response.getDescription());
						commonResponse.setYeepayreponse(response);
					}
					commonResponse.setRequestInfo(outStr);
	 ...
	 * @return
	 */
	public static final CommonResponse useAllaudit(CommonRequst commonRequst){
		logger.info("调用易宝支付直连通用转账授权审核接口开始时间："+new Date());
		Object[]  ret=new Object[3];
		CommonResponse commonResponse=new  CommonResponse();
		try{
			...
			if(yeepay!=null){
				yeepay.setPlatformUserNo(commonRequst.getThirdPayConfigId());//商户平台会员标识
				//取消自动投标授权的属性
				yeepay.setRequestNo(commonRequst.getLoanNoList());//审核流水号
				yeepay.setMode(commonRequst.getConfimStatus()?"CONFIRM":"CANCEL");//CONFIRM 表示解冻后完成资金划转，CANCEL 表示解冻后取消转账
				Map<String, String> params=getReqSign(yeepay,"COMPLETE_TRANSACTION");
				//易宝支付调用地址
				String yeepayurl=yeePayUrl();
				if(yeepayurl!=null){	
